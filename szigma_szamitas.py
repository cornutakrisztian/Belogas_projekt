...
# ==========================================
# --- BEÁLLÍTÁSOK / FÁJL ÉS MUNKALAP NEVEK ---
# ==========================================
file_oszlopkozok = "Oszlopközök(earth)2.xlsx"
file_balmaz = "Ebes MÁV-Berettyóújfalu 1-6. távvezeték bejárási jegyzőkönyv(1-6).xlsx"
file_iee = "IEEE738.xlsb"
file_belogas = "Vezetéksodrony_belógás_KZ_végleges.xlsm"

# Ide írd be a pontos munkalap nevet, ahova az adatokat írni szeretnéd!
cel_lap_neve = "Ebes MÁV-Berettyóújfalu 1-6"
# ==========================================


# 1. FÁJLOK MEGLÉTÉNEK ELLENŐRZÉSE
szukseges_fajlok = {
    "oszlopkozok": file_oszlopkozok,
    "balmaz": file_balmaz,
    "iee": file_iee,
    "belogas": file_belogas
}
...
try:
    ...
    # --- CÉL MUNKALAP KIVÁLASZTÁSA ---
    if cel_lap_neve in [sheet.name for sheet in wb_oszlop.sheets]:
        ws_oszlop = wb_oszlop.sheets[cel_lap_neve]
        print(f"-> Kiválasztott cél fül neve: '{ws_oszlop.name}'")
    else:
        raise ValueError(f"Nem található a '{cel_lap_neve}' nevű munkalap a '{file_oszlopkozok}' fájlban.")
    # ---------------------------------
    ...
    # --- ÚJÍTÁS: A 'J' oszlop formátumának beállítása SZÖVEGRE ("@"), hogy elkerüljük a dátummá alakulást ---
    ws_oszlop.range(f'J2:J{max_sor}').number_format = "@"
